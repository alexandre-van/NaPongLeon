...
class Game_manager:
	...
	def _set_current_game_status(self, game_id):
		...
		game_instance = GameInstance.get_game(game_id).get_game(game_id)
		if game_instance.status and game_instance.status != 'finished' and game_instance.status != 'aborted':
